    ...
    <>
      ...
      >
        <div className={isVisible ? "fade-in" : ""}>
          <h1
            className={`uppercase max-w-8xl mt-8 primary-font text-2xl sm:text-3xl md:text-4xl xl:text-6xl  px-2 pt-3 sm:px-6 border-black border-t border-b-1 ${gStyles.heading}`}
          >
            We are Global
          </h1>
          ...
          <div className={styles.manyRow}>
            <div className={styles.manyItem}>
              {data?.map((val, index) => (
                ...
                >
                  <h1
                    className={`text-lg sm:text-3xl ${gStyles["sub-heading"]}`}
                  >
                    {val?.title}
                  </h1>
                  <h5 className={`text-sm sm:text-xl my-2 ${gStyles["para"]}`}>
                    {val?.desc?.map((v) => (
                      <>
                        {v} <br />{" "}
                      </>
