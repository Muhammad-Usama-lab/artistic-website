...
const ReportIncidentForm = () => {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        onClick={(e) => {
          if (
            modalRef.current &&
      ...
      >
        <div
          ref={modalRef}
          className={`bg-white rounded-2xl shadow-2xl w-full max-w-5xl mx-auto max-h-[90vh] overflow-y-auto transition-transform duration-300 ${isOpen ? "scale-100" : "scale-95"}`}
        >
...
ReportIncidentForm.displayName = "ReportIncidentForm";
